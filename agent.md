...
> **⚠️ Версия v1.15 — стабильная рабочая архитектура.**  
> Дизайн ещё не доработан. При изменениях дизайна откатываться на `git checkout v1.15`.
...
| Файл | Назначение |
|------|-----------|
| `src/main.py` | Главное окно, запись, распознавание, вставка текста, tray |
| `src/hotkey_hook.py` | In-process WH_KEYBOARD_LL хук — горячая клавиша, подавление клавиш, захват |
| `src/hotkey_listener.py` | Старый subprocess-лушатель (не используется) |
| `src/recorder.py` | Запись аудио через sounddevice, очередь сэмплов |
| `src/transcriber.py` | Загрузка моделей (sherpa-onnx/whisper/vosk), распознавание |
